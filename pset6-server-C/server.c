...
int main(int argc, char* argv[])
{
    ...
    while (true)
    {
        ...
        if (connected())
        {
            ...
            // find the first space, what's before will be method
            char* firstSP = strchr(line, ' ');
            // find the second (should be last) space, in-between two will be request-target
            // after last space will be http version
            char* lastSP = strchr(firstSP + 1, ' ');
            // check for extra space
            char* anothSP = strchr(lastSP + 1, ' ');
            // if request-line is not valid
            if ( firstSP == NULL || lastSP == NULL || anothSP != NULL)
            {
                error(400);
                continue;
            }             
            
            // check if method is GET, otherwise 405
            ...
            // check request-target: absolute-path [ "?" query ]
            char reqTrg[lastSP - firstSP];
            strncpy(reqTrg, firstSP + 1, lastSP - firstSP);
            reqTrg[lastSP - firstSP - 1] = '\0';
            // check whether starts with / there is no "
            if (reqTrg[0] != '/' || strchr(reqTrg, '\"') != NULL)
            {
                error(400);
                continue;
            }
            // if absolute-path doesn't contain . return 501
            else if (strchr(reqTrg, '.') == NULL)
            {
                error(501);
                continue;
            }
            // if http version is not HTTP/1.1 return 505
            if (strcmp(lastSP + 1, "HTTP/1.1\r\n") != 0)
            {
                error(505);
                continue;
            } 
            ...
            char* qchar = strchr(reqTrg, '\?');
            int l = 1;
            char query[l];
            
            if (qchar == NULL || (qchar + 1) == '\0')
            {
                query[0] = '\0';
            }
            else
            {
                l = strchr(reqTrg, '\0') - qchar;
                strcpy(query, qchar + 1);
            }
            // concatenate root and absolute-path
            int len = strlen(reqTrg) - strlen(query);
            char path[strlen(root) + len + 1];
            strcpy(path, root);
            strncat(path, reqTrg, strchrnul(reqTrg, '\?') - reqTrg);
            ...
            char* extPtr = strrchr(path, '.') + 1;
            char extension[strlen(extPtr) + 1];
            ...
            else
            {
                ...
                // respond to client
                if (dprintf(cfd, "HTTP/\033[32m1.1 200 OK\033[39m\r\n") < 0)
                {
                    continue;
                }
                if (dprintf(cfd, "Connection: close\r\n") < 0)
...
{   
    ...
    else if (strcasecmp(extension, "html") == 0) return "text/html";
    else if (strcasecmp(extension, "ico") == 0) return "image/x-icon";
    else if (strcasecmp(extension, "jpg") == 0) return "image/jpeg";
    else if (strcasecmp(extension, "js") == 0) return "text/javascript";
    else if (strcasecmp(extension, "png") == 0) return "image/png";
    else return NULL;
}
